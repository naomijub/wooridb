...
use crate::{model::{DataExecutor, DataLocalContext, error::Error}, schemas::query::Response};

use super::query::{select_all_id_when_controller, select_all_with_id, select_keys_id_when_controller, select_keys_with_id};

const ERROR: &str = "Only single value queries are allowed, so key `ID` is required and keys `WHEN AT` are optional";

pub async fn intersect(queries: Vec<Wql>, relation_type: RelationType,local_data: DataLocalContext, actor: DataExecutor,) -> Result<Response, Error> {
...
pub async fn difference(queries: Vec<Wql>, relation_type: RelationType, local_data: DataLocalContext, actor: DataExecutor,) -> Result<Response, Error> {
    let first = queries[0].clone();
    let second = queries[1].clone();
    let first = get_query_value(first, local_data.clone(), actor.clone()).await?;
    let second = get_query_value(second, local_data, actor).await?;
...
pub async fn union(queries: Vec<Wql>, relation_type: RelationType, local_data: DataLocalContext, actor: DataExecutor,) -> Result<Response, Error> {
    let first = queries[0].clone();
    let second = queries[1].clone();
    let first = get_query_value(first, local_data.clone(), actor.clone()).await?;
    let second = get_query_value(second, local_data, actor).await?;
...
async fn get_query_value(query: Wql, local_data: DataLocalContext, actor: DataExecutor) 
